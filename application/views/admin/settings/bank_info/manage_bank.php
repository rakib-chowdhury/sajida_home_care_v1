            ...
            <div class="panel panel-primary" data-collapsed="0">
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-bordered datatable" id="table-1" width="100%">
                        ...
                        <tbody>
                        <?php if (sizeof($banks) != null) { ?>
                            <?php foreach ($banks as $key => $row) { ?>
                                <tr>
                                    <td><?= $row['bank_name'] ?></td>
                                    <td>
                                        <a href="<?= site_url('settings/edit_bank') . '/' . $row['bank_id'] ?>">
                                            <span data-toggle="tooltip" data-placement="top" title="Edit"
                                                  class="glyphicon glyphicon-edit"></span>
                                        </a>

                                        <a data-toggle="modal" data-target="#deleteModal_<?= $row['bank_id'] ?>">
                                            <span data-toggle="tooltip" data-placement="top" title="Delete"
                                                  style="color: red" class="glyphicon glyphicon-trash"></span>
                                        </a>
                                    </td>
                                    <div class="modal fade" id="deleteModal_<?= $row['bank_id'] ?>"
                                         data-backdrop="static">
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <form role="form"
                                                      action="<?= site_url('settings/delete_bank') ?>"
                                                      class="form-horizontal form-groups-bordered" method="post">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Delete Info for <strong><?= $row['bank_name'] ?></strong>?</h4>
                                                    </div>
